...
class PutBlockInContainerUnseenColors(Task):
    ...
    def generate_random_poses(self, env, box_pose, box_dims, obj_dim, num_objs = 1):
        coeffs_x = np.array(random.sample(range(-100, 100, 20), num_objs))/100 * 0.4 * box_dims[0]
        coeffs_y = np.array(random.sample(range(-100, 100, 20), num_objs))/100 * 0.4 * box_dims[1]
        obj_poses = []
